...
<form name="students" @isset($student)
    ...
    <div class="row">
        <div class="form-group col-md-6">
            ...
            <input type="text" name="name" id="name" @isset($student) value="{{ old('name') ?? $student->name }}" @else value="{{ old('name') }}" @endisset class="form-control" placeholder="{{ __('Name') }}" required>
        </div>
        
        <div class="form-group col-md-6">
            <label for="lastName">{{ __('Last Name') }}</label>
            <input type="text" name="last_name" id="lastName" @isset($student) value="{{ old('last_name') ?? $student->last_name }}" @else value="{{ old('last_name') }}" @endisset class="form-control" placeholder="{{ __('Last Name') }}" required>
        </div>
    </div>
    ...
    <div class="row">
        <div class="form-group col-md-6">
            ...
            <input type="email" name="email" id="email" @isset($student) value="{{ old('email') ?? $student->email }}" @else value="{{ old('email') }}" @endisset class="form-control" placeholder="{{ __('Email') }}" required>
        </div>
        
        <div class="form-group col-md-6">
            <label for="phone">{{ __('Phone') }}</label>
            <input type="text" name="phone" id="phone" @isset($student) value="{{ old('phone') ?? $student->phone }}" @else value="{{ old('phone') }}" @endisset class="form-control" placeholder="{{ __('Phone') }}" required>
        </div>
    </div>
    ...
    <div class="row">
        <div class="form-group col-md-6">
            ...
            <input type="text" name="document_number" id="documentNumber" @isset($student) value="{{ old('document_number') ?? $student->document_number }}" @else value="{{ old('document_number') }}" @endisset class="form-control" placeholder="{{ __('Document Number') }}" required>
        </div>
        
        <div class="form-group col-md-6">
            <label for="birthday">{{ __('Birthday') }}</label>
            <input type="text" name="birthday" id="birthday" @if(isset($student) && $student->birthday) value="{{ old('birthday') ?? $student->birthday->format('d-m-Y') }}" @else value="{{ old('birthday') }}" @endif class="form-control" placeholder="{{ __('Birthday') }}" required>
        </div>
    </div>
    ...
    <div class="row">
        ...
        <div class="form-group col-md-6">
            ...
            <select class="form-control" name="city_id" id="cityId">
                @isset($student->city_id)
                    <option value="{{ $student->city_id }}" selected="selected">{{ $student->city->name }}</option>
                @endisset
            </select>
        </div>
    </div>
        ...
        <div class="form-group col-md-6">
            ...
            @if (request()->user()->hasAnyRole(['country-manager','latam-manager','admin']))
                <select class="form-control" name="academy_id" id="academyId">
                    @isset($student->academy_id)
                        <option value="{{ $student->academy_id }}" selected="selected">{{ $student->academy->name }}</option>
                    @endisset
                </select>
            @else
                <select class="form-control" name="academy_id" id="academyId" readonly disabled>
                    <option value="{{ request()->user()->academy_id }}" selected>{{ request()->user()->academy->name }}</option>
                </select>                
            @endif
        </div>    
        ...
        <div class="form-group col-md-6">
            ...
            <select class="form-control" name="grade_id" id="gradeId" required>
                ...
                @foreach($grades as $grade)
                    <option value="{{ $grade->id }}" @if((isset($student) && $student->grade_id == $grade->id) || $grade->id == old('grade_id')) selected @endif>{{ $grade->name }}</option>
                @endforeach
            </select>
        </div>
    </div>
            ...
            @php
                ...
                if (!empty($student->profile_photo)) {
                    ...
                    $preview = [
                        ...
                        'deleteUrl' => route('admin.users.profile-photo.delete', $student->id),
    ...
    <div class="form-group">
        ...
        <div class="checkbox c-checkbox"><label>
            <input type="checkbox" name="active" value="1" @if((isset($student) && $student->active) || old('active')) checked="checked" @endif><span class="fa fa-check"></span> {{ __('Active') }}</label>
        </div>        
    </div>     
    ...
    <div class="row change-password">
        <div class="form-group col-md-6">
            ...
            <input type="password" class="form-control" name="password" id="password" @isset($student) value="" @else required @endisset class="form-control" placeholder="{{ __('Password') }}">
        </div>

        <div class="form-group col-md-6">
            <label for="passwordConfirmation">{{ __('Confirm Password') }}</label>
            <input type="password" name="password_confirmation" id="passwordConfirmation" @isset($student) value="" @else required @endisset class="form-control" placeholder="{{ __('Confirm Password') }}">
        </div>
    </div>        
	...
	<div class="form-group row mt-4">
		<div class="col-6">
			...
			<a href="{{ route('admin.students.index') }}" class="btn btn-sm btn-secondary">{{ __('Cancel') }}</a>
		</div>
	</div>
</form>
