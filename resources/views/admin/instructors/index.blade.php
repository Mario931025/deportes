...
<div class="content-wrapper">
	<div class="content-heading">
        ...
        <ol class="breadcrumb ml-auto">
            ...
            <li class="breadcrumb-item active">{{ __('Instructors') }}</li>
        </ol>        
	</div>
	<div class="row">
		<div class="col-xl-12">
            ...
            <div class="card card-default">
				<div class="card-header">
					<h3>{{ __('Instructors List') }}</h3>
					<div class="row">
                        <div class="col-md-6">
                            <a href="{{ route('admin.instructors.create') }}" class="btn btn-success">{{ __('Create') }}</a>
                        </div>
                        <div class="col-md-6">
                            <form method="GET" id="filter" action="{{ route('admin.instructors.index') }}" accept-charset="UTF-8" class="form-inline float-right" role="search">
                            ...
                            <tbody>
                                ...
                                @foreach ($instructors as $instructor)
                                    <tr>
                                        ...
                                        <td class="text-nowrap">{{ $instructor->last_name }}</td>
                                        <td class="text-nowrap">{{ $instructor->email }}</td>
                                        <td class="text-nowrap">{{ $instructor->phone }}</td>
                                        <td class="text-center">
                                            <a href="{{ route('admin.instructors.edit', $instructor->id) }}" class="btn btn-sm btn-info"><em class="icon-pencil" data-toggle="tooltip" title="{{ __('Edit') }}"></em></a>
                                            <form method="POST" action="{{ route('admin.instructors.destroy', $instructor->id) }}" class="d-inline-block">
                                                @method('DELETE')
                                                @csrf
                ...
                <div class="card-footer">
                    <div class="pagination-wrapper"> {!! $instructors->appends(['search' => Request::get('search')])->links() !!} </div>
                </div>
                 --}}-->
            </div>
		</div>
    ...
    $(document).ready(function() {
        ...
        $dataTables.on('list', function(e, param) {
            ...
            $(this).DataTable({
                ...
                "ajax" : {
                    url: "{{ route('admin.instructors.get') }}",
                    type: "POST",
                    data: param,
                    headers: {
                ...
                "columns": [
                    ...
                    { data: null, width: '10%', render: 'id', orderable: false, className: 'text-center',
                        createdCell: function (td, cellData, rowData, row, col) {
                            ...
                            $('<a>', { href: "{{ url('/') }}/instructors/" + rowData.id + "/edit" })
                            ...
                            if ({{ Auth::user()->id }} !== rowData.id) {
                                var $form = $('<form>', { method: 'POST', action: "{{ url('/') }}/instructors/" + rowData.id })
